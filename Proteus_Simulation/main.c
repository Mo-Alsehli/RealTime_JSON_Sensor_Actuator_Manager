...
void vTestTask_Handler(void *params);
...
int main(void) {
    ...
    // task creation
    if (xTaskCreate(vTestTask_Handler, "UARTTask", 128, NULL, 2, NULL) != pdPASS) {
        printf("Error: Task creation failed!\r\n");
        char *buffer = "Error: Task creation failed!\n";
        MCAL_UART_SendData(UART1, (uint8_t*)buffer, strlen(buffer), enable);
    }
    MCAL_UART_SendData(UART1, (uint8_t*)"Here", 4, enable);
...
void vTestTask_Handler(void *params){
		char* buffer = "Test Task Started Succefully\r\n";
